...
class postDao{
  ...
  public static function GetAllkesye($ident){
            $con=new connexion();
            $cont=$con->executerequete("SELECT p.id_post,a.nom_complet,v.nom_ville, s.adresse_complete,a.etat,a.id_admin,a.email,a.telephone,a.adresse_complete FROM tblsurcussale s
            inner join tbladministration a
            INNER join tblpost p
            INNER JOIN tblville v
            on p.admin_id=a.id_admin  AND p.surcussale_id=s.id_surcussale and v.id_ville=s.ville_id WHERE s.entreprise_id='$ident' and a.type_admin_id=3");
            $con->closeconnexion();
            return $cont;
  }
  ...
  public static function Getkesye($nom){
            ...
            INNER join tblsurcussale s
            INNER JOIN tblville v
            on p.admin_id=a.id_admin  AND p.surcussale_id=s.id_surcussale and v.id_ville=s.ville_id where a.nom_complet='$nom' and a.type_admin_id=3");
            $con->closeconnexion();
            return $cont[0];
  }
